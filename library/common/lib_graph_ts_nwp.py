...
from library.common.lib_graph_ts_utils import configure_ts_axes, configure_ts_attrs, \
...
def plot_ts_discharge_nwp_probabilistic(
    ...
    if attrs_ts_ground_network is not None:
        time_run_ground_network = attrs_ts_ground_network['time_run'].strftime(format=time_format_algorithm)
        section_drained_area = str(attrs_ts_ground_network['section_drained_area'])
        section_name = attrs_ts_ground_network['section_name']
        basin_name = attrs_ts_ground_network['basin_name']
        section_thr_alert = attrs_ts_ground_network['section_discharge_thr_alert']
        section_thr_alarm = attrs_ts_ground_network['section_discharge_thr_alarm']
    else:
        raise IOError('Ground network dataframe attributes not found')

    if attrs_ts_nwp_det is not None:
        time_run_nwp_det = attrs_ts_nwp_det['time_run'].strftime(format=time_format_algorithm)
    else:
        raise IOError('NWP deterministic dataframe attributes not found')

    if attrs_ts_nwp_prob is not None:
        time_run_nwp_prob = attrs_ts_nwp_prob['time_run'].strftime(format=time_format_algorithm)
        time_restart = attrs_ts_nwp_prob['time_restart'].strftime(format=time_format_algorithm)
        time_start = attrs_ts_nwp_prob['time_start'].strftime(format=time_format_algorithm)
        run_name = attrs_ts_nwp_prob['run_name']
    else:
        raise IOError('NWP probabilistic dataframe attributes not found')

    # Compute nwp probabilistic peak(s)
    df_discharge_simulated_peaks_probabilistic = compute_ts_peaks(
        df_discharge_simulated_nwp_probabilistic, peak_value_min=0, peak_timestr_min=time_run_nwp_prob)
    ...
    p211 = ax2.axhline(section_thr_alert, color='#FFA500', linestyle='--',
                       linewidth=2, label=tag_discharge_thr_alert)
    p212 = ax2.axhline(section_thr_alarm, color='#FF0000', linestyle='--',
                       linewidth=2, label=tag_discharge_thr_alarm)
...
def plot_ts_discharge_nwp_deterministic(
    ...
    if attrs_ts_ground_network is not None:
        ...
        raise IOError('Ground network dataframe attributes not found')
    if attrs_ts_nwp is not None:
        time_run_nwp = attrs_ts_nwp['time_run'].strftime(format=time_format_algorithm)
        time_restart = attrs_ts_nwp['time_restart'].strftime(format=time_format_algorithm)
        time_start = attrs_ts_nwp['time_start'].strftime(format=time_format_algorithm)
        run_name = attrs_ts_nwp['run_name']
    else:
        raise IOError('NWP dataframe attributes not found')
    ...
    p22 = ax2.plot(df_discharge_simulated_ground_network.index, df_discharge_simulated_ground_network.values[:, 0],
                   color='#0000FF', linestyle='-', lw=1)
    p23 = ax2.plot(df_discharge_simulated_nwp.index, df_discharge_simulated_nwp.values[:, 0],
                   color='#0066ff', linestyle='-', lw=1)
    ...
    p29 = ax2.axhline(section_thr_alert, color='#FFA500', linestyle='--',
                      linewidth=2, label=tag_discharge_thr_alert)
    p210 = ax2.axhline(section_thr_alarm, color='#FF0000', linestyle='--',
                       linewidth=2, label=tag_discharge_thr_alarm)
